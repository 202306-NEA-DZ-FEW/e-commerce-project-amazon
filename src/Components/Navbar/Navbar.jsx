...
const Navbar = () => {
  ...
  <Link href={"/"}><AmazonLogo/></Link>
  <div className="flex">
    <ul className="flex uppercase items-center gap-8">
      <li className="px-4 hover:text-red-700"><Link href={"/"}>men's clothing</Link></li>
      <li className="px-4 hover:text-red-700"><Link href={"/"}>women's clothing</Link></li>
      <li className="px-4 hover:text-red-700"><Link href={"/"}>jewlery</Link></li>
      <li className="px-4 hover:text-red-700 relative"><DropDown/></li>
...
<nav className="h-[80%]">
  <div className="text-xl z-10" onClick={() => setisOpen(!isopen)}>
    {isopen ? (<IoMdClose/>) : (<GiHamburgerMenu/>)}
    <div className="bg-white fixed w-full ">
    <ul className={`bg-bkg fixed  w-64 z-20 top-0 overflow-y-auto bottom-0 py-24 pl-4 duration-500 left-0 ${isopen ? "right-0" : "right-[-100%]"}`}>
